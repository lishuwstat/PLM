## code for two covariates with non-linear effects
...
gen.data<-function(n,fun,r,gt,tau){
  ...
  for(i in 1:n){
    ...
    Num = min(which(Sn>tau))-1 # number of observations
  ...
  L[d1==1]<-NA
  R[d3==1]<-NA
  sdata<-data.frame(Li=L,Ri=R,d1=d1,d2=d2,d3=d3,X1=X1,X2=X2,Ti=T,W1=W1,W2=W2)
  return(sdata)
}
